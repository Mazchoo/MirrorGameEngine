...
from Helpers.VertexDataOperations import (
    normalize_l1,
    get_bbox_2d,
    centroid_weighted_by_face,
    convex_volume,
)
...
class ObjMtlMesh:
    ...
    def __init__(
        ...
        color_variation: Optional[dict] = None,
        **kwargs,
    ):
        ...
        for material_data in mtl_dict.values():
            ...
            if texture is None:
                ...
                self.draw_iterator.append(
                    (material, texture["count"], texture["offset"])
                )
    ...
    def update_bbox_and_centroid(self, player: Player):
        ...
        bbox = np.vstack(
            [
                [
                    new_v_min,
                    [new_v_min[0], new_v_max[1]],
                    new_v_max,
                    [new_v_max[0], new_v_min[1]],
                ]
            ]
        )
        ...
        rot_mat = np.array(
            [[np.cos(rot), -np.sin(rot)], [np.sin(rot), np.cos(rot)]], dtype=np.float32
        )
