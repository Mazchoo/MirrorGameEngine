...
from Helpers.Globals import (
    SPECULAR_EXPONENT_WEIGHTING,
    MIN_SPECULAR_EXPONENT,
...
class ShinyMaterial:
    ...
    def __init__(self, hue_offset, **kwargs):
        self.ambient_weighting = get_value_or_default(
            kwargs, "ambient_weighting", DEFAULT_AMBIENT_WEIGHTING
        )
        self.diffuse_weighting = get_value_or_default(
            kwargs, "diffuse_weighting", DEFAULT_DIFFUSE_WEIGHTING
        )
        self.specular_weighting = get_value_or_default(
            kwargs, "specular_weighting", DEFAULT_SPECULAR_WEIGHTING
        )
        self.specular_exponent = get_value_or_default(
            kwargs, "specular_exponent", DEFAULT_SPECULAR_EXPONENT
        )
        self.specular_exponent = max(
            self.specular_exponent * SPECULAR_EXPONENT_WEIGHTING, MIN_SPECULAR_EXPONENT
        )
        self.opacicty = get_value_or_default(kwargs, "opacicty", DEFAULT_OPACITY)
        self.specular_tint = get_value_or_default(
            kwargs, "specular_tint", DEFAULT_SPECULAR_TINT
        )
        ...
        image_data, width, height = read_image_data_from_source(
            kwargs["texture"], hue_offset
        )
    ...
    def assign_global_slots(self, shader, **kwargs):
        ...
        for global_id_name in GLOBAL_ID_NAMES:
            if global_name := get_value_or_default(kwargs, global_id_name, None):
                self.__setattr__(
                    global_id_name, glGetUniformLocation(shader, global_name)
                )
